...
export default function GeneralCategoriesPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(`/api/admin/general-categories?categoryId=${selectedCategoryId}`, {
        method: 'GET',
        headers: {
      ...
      if (response.status === 401) {
        router.push('/login?redirectTo=/admin/general-categories');
        return;
      }
  ...
  const handleDeleteScore = async (judgeId: string, scoreId: string) => {
    ...
    try {
      ...
      const response = await fetch('/api/admin/general-categories/results', {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleSaveScores = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/admin/general-categories', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    return (
      <div className="flex-1 p-4">
        <div className="bg-white p-8 rounded-lg shadow-lg flex flex-col items-center max-w-md mx-auto">
          ...
          <p className="text-gray-600">Please wait while we fetch the selected submissions...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-gray-900">General Categories - Jury Scoring</h1>
                <p className="text-gray-600 text-sm">
                  Manual score entry for selected submissions (flexible quantity)
                </p>
              </div>
            </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
          <div className="flex items-start">
            ...
            <div>
              <h3 className="font-semibold text-blue-800 mb-1">Manual Score Entry for General Categories</h3>
              <p className="text-blue-700 text-sm">
                Select a submission, choose a judge, and enter their scores for each criterion.
                The scoring criteria and weights are the same as the city project category.
              </p>
            </div>
          </div>
        </div>
